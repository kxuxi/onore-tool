...
- **戦闘履歴タブ**: タブ区切りの戦闘履歴を貼り付けて「登録する」で両陣営の武将を一括登録。並び替え（新しい順 / 古い順）・国での絞り込み・キーワード検索に対応。登録に成功すると入力欄は自動でクリアされます
- **偵察検索タブ**: スペース / 改行 / カンマ区切りで武将名を貼り付けてタイプ・兵科・兵種を即座に表示。国へ報告するための「名前［タイプ｜兵種］」形式のテキスト（件数つき）も生成・コピーできます
- **被弾表タブ**: 登録済みデータから各武将の行動状況（行動可 / 不明 / 行動済み）を集計表示。時刻は自動更新されます
- **武将ランキングタブ**: 出兵勝利数 / 守備勝利数 / SWI（攻撃）/ SWI（守備）の4指標で武将を順位表示。兵科・武将名・最低出撃数で絞り込み
- **DB確認タブ**: 登録済み武将の一覧 / 名前や国・タイプ・兵科・兵種で絞り込み。列ヘッダーで並び替え、結果を TSV でコピー可能
- **兵種図鑑タブ**: 兵種マスタ（攻撃・防御・コスト・特殊攻撃など）の閲覧・追加・編集
- **武器・品物タブ**: 戦闘履歴の装備データから武器・品物の使用回数・勝率・主な使用武将を集計表示
...
- **共有リンク**: 表示中のタブ・詳細ページは URL に反映され、ヘッダーや詳細ページの「共有 / リンクをコピー」ボタン、再読込でそのまま復元できます
